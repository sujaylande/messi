...
function StudentPublic() {
  ...
  const fetchNotices = async () => {
    try {
      const res = await axios.get("http://localhost:5001/api/student/display-notices")
      setNotices(res.data)
    } catch (error) {
  ...
  const fetchMenu = async () => {
    try {
      const res = await axios.get("http://localhost:5001/api/student/display-menu")
      setMenu(res.data)
    } catch (error) {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      setRegNo("")
      // Switch to attendance tab after successful fetch
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault()
    try {
      const response = await axios.post("http://localhost:5001/api/student/feedback-form", formData)
      ...
      <main className="container mx-auto px-4 py-8">
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center mb-2">
                      ...
                      <h3 className="ml-2 text-lg font-semibold text-gray-800">{m.meal_slot}</h3>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            {notices.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                {notices.map((n) => (
                  <li key={n.id} className="p-4 hover:bg-blue-50 transition-colors">
        ...
        {activeTab === "attendance" && (
          <div>
            ...
            {activeTab === "attendance" && !loading && !error && !attendanceData.length && !studentDetails && (
