...
// // Lazy loaded components can be added here if needed
...
//   const [attendancePerPage] = useState(7)
...
//   // React Query for mess statistics data
//   const {
//     data: messData = [],
...
//   // Effect to maintain scroll position when switching tabs
//   useEffect(() => {
//     const fetchData = async () => {
//       if (studentListRef.current) {
//         // Save the current scroll position
//         const scrollPosition = studentListRef.current.getBoundingClientRect().top
...
//         if (studentListRef.current) {
//           window.scrollTo({
//             top: window.scrollY + studentListRef.current.getBoundingClientRect().top - scrollPosition,
//             behavior: "instant",
//           })
//         }
//       }
...
//   // Pagination logic for attendance table
...
//   const sortedAttendanceData = getSortedData(messData)
//   const currentAttendanceData = sortedAttendanceData.slice(indexOfFirstAttendance, indexOfLastAttendance)
//   const totalAttendancePages = Math.ceil(sortedAttendanceData.length / attendancePerPage)

//   // Pagination logic for students table
//   const indexOfLastStudent = studentsPage * studentsPerPage
...
//   // Pagination handlers
//   const paginateAttendance = (pageNumber) => setAttendancePage(pageNumber)
//   const paginateStudents = (pageNumber) => setStudentsPage(pageNumber)

//   // FIXED: Calculate meal trends (increase/decrease from previous day)
//   const calculateTrend = (index, mealType) => {
//     // Get the actual index in the original data array
//     const dataIndex = indexOfFirstAttendance + index

//     // If this is the first item or there's no previous item, return null
//     if (dataIndex >= sortedAttendanceData.length - 1) return "neutral"

//     const currentCount = sortedAttendanceData[dataIndex][`${mealType}_count`]
//     const previousCount = sortedAttendanceData[dataIndex + 1][`${mealType}_count`]
...
//   const renderPagination = (currentPage, totalPages, paginate, type) => {
//     return (
//       <div className="px-6 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
//         <div className="text-sm text-gray-500">
//           {type === "attendance" ? (
//             <>
//               Showing {indexOfFirstAttendance + 1} to {Math.min(indexOfLastAttendance, sortedAttendanceData.length)} of{" "}
//               {sortedAttendanceData.length} entries
//             </>
//           ) : (
//             <>
...
//               >
//                 <strong className="font-bold">Error!</strong>
//                 <span className="block sm:inline"> Failed to load mess statistics. Please try again.</span>
//               </div>
//             ) : (
//               <>
//                 <table className="w-full">
...
//                       currentAttendanceData.map((row, index) => (
//                         <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
//                           <td className="py-3 px-6 text-left whitespace-nowrap font-medium">{row.formatted_date}</td>
//                           <td className="py-3 px-6 text-center">
//                             <span className="bg-blue-100 text-blue-800 py-1 px-3 rounded-full text-xs">
...
//                 {sortedAttendanceData.length > attendancePerPage &&
//                   renderPagination(attendancePage, totalAttendancePages, paginateAttendance, "attendance")}
//               </>
//             )}
//           </div>
//         </div>
...
//                   onClick={() => {
//                     setFilter("active")
//                     setStudentsPage(1) // Reset to first page on filter change
//                   }}
//                   className={`px-3 py-1 rounded-md text-sm font-medium transition-colors ${
//                     filter === "active" ? "bg-white text-blue-700" : "bg-blue-400 text-white hover:bg-blue-300"
//                   }`}
...
//                   renderPagination(studentsPage, totalStudentPages, paginateStudents, "students")}
//               </>
//             )}
//           </div>
//         </div>
