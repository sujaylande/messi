...
function AttendanceTab({ getStudentFromStorage }) {
  ...
  // Use React Query with a stale time of 1 hours (attendance updates ~4 times/day)
  const { data, isLoading, error } = useQuery({
    queryKey: ["attendance", currentStudent?.reg_no, currentStudent?.block_no],
    queryFn: async () => {
      if (!currentStudent?.reg_no || !currentStudent?.block_no) {
        throw new Error("Student information not found. Please log in again.")
      }

      const response = await studentAxios.get(`/student-stat/${currentStudent.reg_no}/${currentStudent.block_no}`)
      ...
      return {
        ...
        studentDetails: response.data.studentDetails || response.data.studentwithoutpassword,
      }
    },
    staleTime: 1000 * 60 * 60 * 1, // 1 hours
    enabled: !!currentStudent?.reg_no && !!currentStudent?.block_no,
    retry: 1,
  })

  // Download attendance as CSV
  const downloadAttendanceCSV = useCallback(() => {
    if (!data?.attendanceData || data.attendanceData.length === 0) return

    const headers = ["Date", "Breakfast", "Lunch", "Snack", "Dinner", "Total Cost"]
    const csvRows = []
    csvRows.push(headers.join(","))

    for (const row of data.attendanceData) {
      const values = [
        row.formatted_date,
        row.breakfast_time || "--",
        row.lunch_time || "--",
    ...
    csvRows.push(`Total Amount,,,,,₹${data.totalAmount}`)
    ...
    a.setAttribute("hidden", "")
    a.setAttribute("href", url)
    a.setAttribute("download", `attendance_${data.studentDetails?.reg_no || "data"}.csv`)
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
  }, [data])
  ...
  if (error) {
    return (
      <div className="bg-red-100 text-red-700 p-4 rounded-md mb-6">
        ...
        <button
          onClick={() =>
            queryClient.invalidateQueries(["attendance", currentStudent?.reg_no, currentStudent?.block_no])
          }
          className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 block mx-auto"
        >
          Try Again
  ...
  if (!data?.studentDetails && (!data?.attendanceData || data.attendanceData.length === 0)) {
    return (
      <div className="bg-white rounded-lg shadow-md p-8 text-center">
        <User className="h-16 w-16 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-700 mb-2">No Student Data</h3>
